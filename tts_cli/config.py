...
def load_config() -> Dict[str, Any]:
    """Load legacy JSON configuration from file, returning defaults if file doesn't exist or is corrupt."""
...
def validate_api_key(provider: str, api_key: str) -> bool:
    ...
    if provider == "openai":
        ...
        return (api_key.startswith("AIza") and len(api_key) == get_config_value('google_api_key_length', 39)) or \
               (api_key.startswith("ya29.") and len(api_key) > get_config_value('oauth_token_min_length', 50)) or \
               len(api_key) > get_config_value('service_account_json_min_length', 100)  # Service account JSON string
