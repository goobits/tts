...
class TestExceptionHierarchy:
    ...
    def test_exception_raising_and_catching(self):
        ...
        # Test catching by base TTSError
        with pytest.raises(TTSError):
...
class TestErrorMessageFormatting:
    ...
    def test_error_type_consistency(self):
        ...
        for _ in range(5):  # Test multiple times
            assert isinstance(map_http_error(401), AuthenticationError)
            assert isinstance(map_http_error(429), RateLimitError)
            assert isinstance(map_http_error(500), ServerError)
            assert isinstance(map_http_error(400), ProviderError)
