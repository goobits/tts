...
class TestCleanupFile:
    ...
    def test_cleanup_special_characters_in_path(self):
        """Test cleanup with special characters in file path."""
        # Create temporary files with special names
        special_names = [
    ...
    def test_cleanup_empty_string_path(self):
        """Test cleanup with empty string path."""
        # Should not raise exception
        cleanup_file("")
        cleanup_file("   ")  # Whitespace only
...
class TestAudioUtilityIntegration:
    ...
    def test_concurrent_file_operations(self):
        ...
        try:
            ...
            for i in range(3):
                with tempfile.NamedTemporaryFile(delete=False, suffix=f'_concurrent_{i}.wav') as tmp:
                    temp_files.append(tmp.name)
...
class TestPathHandling:
    ...
    def test_unicode_paths(self):
        ...
        for name in unicode_names:
            try:
                with tempfile.NamedTemporaryFile(delete=False, suffix=name, prefix="test_") as tmp:
                    temp_path = tmp.name
                    tmp.write(b"unicode test")
                
                # Test that cleanup works with unicode paths
                assert os.path.exists(temp_path)
                cleanup_file(temp_path)
                assert not os.path.exists(temp_path)
                ...
                # Skip if the filesystem doesn't support this name
                continue
    
    def test_very_long_paths(self):
        """Test handling of very long file paths."""
        # Create a long but valid filename
        long_name = "a" * 100 + ".wav"  # 104 characters
        
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=long_name, prefix="test_") as tmp:
                temp_path = tmp.name
                tmp.write(b"long path test")
            ...
            pytest.skip("System doesn't support long file paths")
