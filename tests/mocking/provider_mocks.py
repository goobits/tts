...
class ElevenLabsMockProvider:
    ...
    def setup(registry: NetworkMockRegistry) -> None:
        ...
        # Voice list endpoint
        registry.register_pattern("api.elevenlabs.io/v1/voices", ElevenLabsMockProvider._handle_voices)

        # Text-to-speech endpoint
        registry.register_pattern("api.elevenlabs.io/v1/text-to-speech", ElevenLabsMockProvider._handle_synthesis)
...
class GoogleTTSMockProvider:
    ...
    def _handle_synthesis(method: str, url: str, kwargs: Dict[str, Any]) -> MockHTTPResponse:
        ...
        response_data = {"audioContent": audio_b64}
...
def mock_edge_tts_api(monkeypatch):
    ...
    return {"Communicate": mock_communicate, "list_voices": mock_list_voices}


@pytest.fixture
def mock_all_provider_apis(mock_elevenlabs_api, mock_openai_api, mock_google_api, mock_edge_tts_api):
...
def create_api_key_error_scenario(registry: NetworkMockRegistry, provider: str) -> None:
    ...
    if provider == "elevenlabs":
        registry.register_pattern(
            "api.elevenlabs.io", lambda method, url, kwargs: create_error_response(401, "Invalid API key")
        )
    elif provider == "google":
        registry.register_pattern(
            "texttospeech.googleapis.com", lambda method, url, kwargs: create_error_response(401, "Invalid API key")
        )
