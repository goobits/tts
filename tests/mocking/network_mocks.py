...
def create_json_response(
    ...
    return MockHTTPResponse(
        status_code=status_code, content=json_text.encode(), json_data=data, text=json_text, headers=response_headers
    )


def create_audio_response(
    audio_data: bytes = b"mock_audio_data", status_code: int = 200, content_type: str = "audio/mpeg"
) -> MockHTTPResponse:
    """Create an audio response."""
    return MockHTTPResponse(status_code=status_code, content=audio_data, headers={"Content-Type": content_type})


def create_error_response(
    status_code: int, error_message: str, error_details: Optional[Dict[str, Any]] = None
) -> MockHTTPResponse:
...
def create_streaming_response(
    chunks: List[bytes], status_code: int = 200, content_type: str = "audio/mpeg"
) -> MockHTTPResponse:
    """Create a streaming response that returns chunks."""
    total_content = b"".join(chunks)

    response = MockHTTPResponse(status_code=status_code, content=total_content, headers={"Content-Type": content_type})
